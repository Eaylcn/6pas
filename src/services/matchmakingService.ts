 ...
 */
export async function findOpponent(playerPower: number, playerTeamName: string): Promise<TeamMatchInfo> {
  const delay = 1000 + Math.random() * 2000;
  await new Promise((resolve) => setTimeout(resolve, delay));
  const rng = createRng(randomSeed());
  return findMatchForRun(rng, playerPower, playerTeamName);
}
